...
help='Comma-separated error model flags (0, 0.5, 1)', default="0.1")
...
for j in pred_lengths:
    for i in error_flags:
        if args.seasonal_trend:
            if "ETT" in args.data:
                cmd = f"bash ./scripts/long_term_forecast/ETT_script/{args.model}_{args.data}_test.sh 8 1 {j} {i} {args.ecm} {args.season_coef} {args.trend_coef}"
            else:
                cmd = f"bash ./scripts/long_term_forecast/{args.data}_script/{args.model}_test.sh 8 1 {j} {i} {args.ecm} {args.season_coef} {args.trend_coef}"
        else:
            if "ETT" in args.data:
