...
for j in pred_lengths:
    for i in error_model:
        if "ETT" in args.data:
            cmd = f"bash ./scripts/long_term_forecast/ETT_script/{args.model}_{args.data}_test.sh 4 1 {j} {i}"
        else:
            cmd = f"bash ./scripts/long_term_forecast/{args.data}_script/{args.model}_test.sh 4 1 {j} {i}"
